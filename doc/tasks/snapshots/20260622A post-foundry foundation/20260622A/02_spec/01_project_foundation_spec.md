...
非敏感配置可以放在 `config/*.yaml` 或数据库中，并由 Zod schema 校验。敏感配置不能写入普通配置文件，也不能写入 task 文档或测试 fixture。v0 默认从本地 secrets 文件读取敏感配置，默认路径为 `secrets/accounts.local.json`；该文件必须被 Git / jj 忽略，不能提交。环境变量只作为 CI、一次性 debug 或临时覆盖入口，不作为日常运行的唯一凭据来源。
...
第三方 X 数据 API 只读公开数据。第一候选是 TwitterAPI.io，它适合关键词搜索、用户公开 posts、评论和公开资料读取。热点扫描、公开账号观察和 topic source 相关读取都归公开 X 数据 provider port 承载，不走 X 官方 API。第三方数据 API 的成本风险来自查询次数、返回条数、分页规模和 provider 计费规则；第一子任务只允许小样本 smoke，正式查询预算在账号配置和后续 topic task 中冻结。第三方数据 API 不能获得 X 密码、cookie 或登录态，也不能执行点赞、关注、回复或发帖。
...
`just test-api-offline` 是正式 Gate，可以进入默认测试链路。`just debug-api-online` 是手动入口，需要本地 secrets 文件或显式临时覆盖，不进入默认测试。
...
* 在线 API 诊断必须放在 `debug/` 或等价目录，通过明确 `just debug...` 命令运行。
...
在线验收不替代离线 Gate。若环境没有 API key 或用户不允许联网，最终报告必须写清未执行在线 debug 的原因和残余风险。
